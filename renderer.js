...
on(screenReadBtn, 'click', async () => {
  ...
  // --- SIMPLE TOGGLE (only one button, no extra logic) ---
  const isActive = screenReadBtn.classList.contains('active');

  if (isActive) {
    // turn OFF
    screenReadBtn.classList.remove('active');
    setState('idle');
    appendLog('[screen] Screen Read OFF');
    return;
  }

  // turn ON
  screenReadBtn.classList.add('active');
  appendLog('[screen] Screen Read ON');
